...
Rejected edits surface on two paths, both fed by the same `validateDevice` rule. Every validated editor runs under `invalidEditValueMode="block"`: the custom name editor reports errors through `useGridCellEditor`, and the provided number and select editors through `cellEditorParams.getValidationErrors`, which a shared adapter in `columns.ts` answers with `fieldError` from `model.ts`. Enter on an invalid draft does not close the editor; the grid marks the input `aria-invalid` and announces the error through its live region, and the name editor also keeps a persistent description. Escape abandons the draft explicitly, and Save refuses to run while an invalid editor is still open. Moving focus to a control outside the screen, for example the navigation or the theme switch, cancels an editor that block mode still holds; clicking content inside the screen, which focuses the page's main region, leaves it blocked. Without that cancel, leaving by mouse could destroy the grid while AG Grid's React wrapper is re-attaching the editor, which raised an uncaught error (`e2e/remediation.spec.ts` › "cancels an invalid name editor when focus leaves the configuration screen", › "keeps an invalid name editor blocked when content inside the configuration screen is clicked" and › "leaves Device Configuration by mouse while an invalid name editor is open, without a page error"). Writes that bypass an editor, such as a Delete-key clear of a threshold, are validated again in the `valueSetter`, rejected, and shown as a message with cell styling. Within the screen, neither path discards a value silently.

The invariant is `warningThreshold < criticalThreshold`; sampling is an integer from 1 to 3,600 seconds. Accepted edits mutate the draft rows in place, so the row array stays stable across edits and native edit undo/redo keeps its history; the screen re-renders dirty metadata from the store snapshot. The saved baseline is copied independently. Save is pessimistic: editing/actions are restricted while the simulated request is in flight, and the baseline advances only on success. Failed saves retain drafts.

Sharing draft and baseline objects would let a grid edit mutate both and incorrectly clear dirty state. Pessimistic saves avoid optimistic rollback and concurrent-snapshot reconciliation while still showing the draft immediately. Optimistic persistence would suit a latency-sensitive workflow with defined version/conflict handling. Cell-level validation means critical may need raising before warning; full-row editing would allow validating both new thresholds together. The configuration store keeps the grid as the edit path so native undo still works; routing edits through `readOnlyEdit` and `onCellEditRequest` would move undo into the store as well.

Adding a row marks it dirty. Deletion requires confirmation and is staged; Save all commits staged deletions and Revert all restores them before saving. Native undo/redo handles cell edits, not the complete application transaction history. Sorting, filtering, row replacement and column movement, pinning or visibility changes clear native undo stacks. Columns and Reset State can therefore clear edit history too. Leaving the screen destroys the grid, so its undo history starts empty on return (`e2e/remediation.spec.ts` › "keeps a renamed draft across navigation, but native cell Undo resets once the grid remounts").

The shell creates one configuration store (`store.ts`) per app session and passes it to the screen, which reads it with `useSyncExternalStore` and unmounts on navigation. Drafts, the baseline, validation errors and an in-flight save stay in the store (`App.integration.test.tsx` › "fully unmounts the configuration grid when navigating away, and restores the draft and dirty count from the store on return"). The shell also owns the `beforeunload` warning, so it stays active while the screen is unmounted (› "keeps the unsaved-changes warning active while Configuration is unmounted, and drops it after a revert"). Reload starts a fresh mock session. In production, saved configuration would come from an API and draft durability would be an explicit product decision.

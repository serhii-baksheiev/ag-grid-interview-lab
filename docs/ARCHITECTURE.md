...
Rejected edits surface on two paths, both fed by the same `validateDevice` rule. Every validated editor runs under `invalidEditValueMode="block"`: the custom name editor reports errors through `useGridCellEditor`, and the provided number and select editors through `cellEditorParams.getValidationErrors`, which a shared adapter in `columns.ts` answers with `fieldError` from `model.ts`. Enter on an invalid draft does not close the editor; the grid marks the input `aria-invalid` and announces the error through its live region, and the name editor also keeps a persistent description. Escape abandons the draft explicitly, and Save refuses to run while an invalid editor is still open. Moving focus out of the screen, for example to the navigation or the theme switch, commits what the editors allow and cancels an editor that block mode still holds. Otherwise leaving by mouse could destroy the grid while AG Grid's React wrapper is re-attaching that editor, which raised an uncaught error (`e2e/remediation.spec.ts` › "cancels an invalid name editor when focus leaves the configuration screen" and › "leaves Device Configuration by mouse while an invalid name editor is open, without a page error"). Writes that bypass an editor, such as a Delete-key clear of a threshold, are validated again in the `valueSetter`, rejected, and shown as a message with cell styling. Neither path discards a value silently.

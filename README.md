...
Vitest covers domain rules, query correctness, datasource lifecycle, malformed storage and component interactions. Playwright runs the same suite in Chromium and Firefox against a dedicated production preview on port **4176**; it refuses to reuse an unrelated server. `npm run test:e2e` builds first so the preview never serves a stale `dist/`; CI builds once and runs `npm run test:e2e:ci`, which only starts the preview. To run one browser against an existing build, pass its project to that script: `npm run test:e2e:ci -- --project=firefox`. Tests cover retry, typing races, theme persistence, keyboard editing, narrow viewports and axe accessibility checks, and every test fails on an uncaught exception, console error or AG Grid warning.

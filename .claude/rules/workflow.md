...
- The full suite runs in CI and, before a session may end, through the
  ...
  still refuses `--no-verify` / `git commit -n` at the tool layer: the flag is
  refused so a bypass never becomes routine, not because a check exists for it
  to bypass. If a commit-time check is ever added, that refusal is already in
  place; until then, the checks that gate a change are the DoD gate and CI.
